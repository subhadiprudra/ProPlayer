...
public class Matches extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        textView.setText("No matches are available for you");

        adapter= new MatchesAdapter(matchModelList,getContext(),"matches");
        ...
        fdb.child("matches").addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for (DataSnapshot data : dataSnapshot.getChildren()){
                    ...
                    if(matchModel.getStatus().equals("locked") || matchModel.getStatus().equals("dead")|| matchModel.getStatus().equals("released")){
                        continue;
                    }
                    ...
                    for(int i=0;i<joinList.size();i++){
                        try {
                            if (joinList.get(i).getAccountId().equals(basicFunctions.read("id")) && joinList.get(i).getMatchId().equals(matchModel.getMatchId())) {
                                matchModelList.remove(matchModel);
                                break;
                            }
                        }catch (Exception e){
